...
mod tests {
    ...
    fn test_package_available() -> Result<(), Box<dyn Error>> {
        let mock = MockCommandRunner::builder()
            .expect("apt-cache show package")
            .status(0)
            .finish()
            .build();

        let apt = AptManager::new(Arc::new(mock.clone()));
        let result = apt.package_available("package")?;
        assert!(result);
    ...
    fn test_install() -> Result<(), Box<dyn Error>> {
        ...
        let apt = AptManager::new(Arc::new(mock.clone()));
        let result = apt.install_package("package");
        assert!(result.is_ok());
    ...
    fn test_is_installed() -> Result<(), Box<dyn Error>> {
        ...
        let apt = AptManager::new(Arc::new(mock.clone()));
        let result = apt.is_package_installed("package")?;
        assert!(result);
