...
mod tests {
    ...
    fn test_package_available() -> Result<(), Box<dyn Error>> {
        let mock = MockCommandRunner::builder()
            .expect("brew info package")
            .status(0)
            .finish()
            .build();

        let brew = BrewManager::new(Arc::new(mock.clone()));
        let result = brew.package_available("package")?;
        assert!(result);
    ...
    fn test_install() -> Result<(), Box<dyn Error>> {
        ...
        let brew = BrewManager::new(Arc::new(mock.clone()));
        let result = brew.install_package("package");
        assert!(result.is_ok());
    ...
    fn test_is_installed() -> Result<(), Box<dyn Error>> {
        ...
        let brew = BrewManager::new(Arc::new(mock.clone()));
        let result = brew.is_package_installed("package")?;
        assert!(result);
